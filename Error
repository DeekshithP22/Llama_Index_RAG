...
your credentials and persona selection.")
...
def display_main_page(persona):
    st.title(f"{persona.capitalize()} Page")
    st.write("Welcome to the main page!")
    st.write("Here you can ask questions and get answers.")
...
def home_page():
    ...
    if st.sidebar.button("Login"):
        if authenticate(email, password, persona):
            st.sidebar.success("Authentication successful!")
            st.session_state.authenticated = True
            st.session_state.email_input = ""  # Clear email field
            st.session_state.password_input = ""  # Clear password field
        else:
            st.sidebar.error("Authentication failed. Please check your credentials and persona selection.")
